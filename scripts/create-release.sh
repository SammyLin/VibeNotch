...
SCHEME="vibenotch"
PROJECT_PATH="vibenotch/vibenotch.xcodeproj"
...
# Create one with: xcrun notarytool store-credentials "vibenotch-notarize" --apple-id "you@example.com" --team-id "YOUR_TEAM_ID"
NOTARYTOOL_PROFILE="vibenotch-notarize"
...
NOTARIZE_ZIP="${BUILD_DIR}/vibenotch-submit.zip"
...
"$GENERATE_APPCAST" \
    ...
    --download-url-prefix "https://github.com/sk-ruban/vibenotch/releases/download/v${VERSION}/" \
    --release-notes-url-prefix "https://updates.vibenotch.app/sparkle-notes/" \
    -o "$APPCAST_OUTPUT" \
    "$APPCAST_STAGING"
