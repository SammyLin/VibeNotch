...
private let logger = Logger(subsystem: "com.vibenotch.app", category: "KeychainManager")
...
enum KeychainManager {
    ...
    private static let vibeNotchService = "com.vibenotch.app"
    ...
    static func setAnthropicApiKey(_ key: String?) {
        if let key, !key.isEmpty {
            saveString(key, service: vibeNotchService, account: anthropicApiKeyAccount)
        } else {
            deleteItem(service: vibeNotchService, account: anthropicApiKeyAccount)
        }
    }
    ...
    static func cacheOAuthToken(_ token: String) {
        saveString(token, service: vibeNotchService, account: cachedOAuthTokenAccount)
    }

    static func clearCachedOAuthToken() {
        deleteItem(service: vibeNotchService, account: cachedOAuthTokenAccount)
    }
