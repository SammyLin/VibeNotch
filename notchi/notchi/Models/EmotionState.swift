...
private let logger = Logger(subsystem: "com.vibenotch.app", category: "EmotionState")

@MainActor
@Observable
final class EmotionState {
    private(set) var currentEmotion: VibeNotchEmotion = .neutral
    private(set) var scores: [VibeNotchEmotion: Double] = [
        .happy: 0.0,
        .sad: 0.0
    ]
    ...
    func recordEmotion(_ rawEmotion: String, intensity: Double, prompt: String) {
        let emotion = VibeNotchEmotion(rawValue: rawEmotion)
