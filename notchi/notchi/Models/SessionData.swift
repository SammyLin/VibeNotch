...
private let logger = Logger(subsystem: "com.vibenotch.app", category: "SessionData")
...
final class SessionData: Identifiable {
    ...
    private(set) var task: VibeNotchTask = .idle
    let emotionState = EmotionState()
    var state: VibeNotchState {
        VibeNotchState(task: task, emotion: emotionState.currentEmotion)
    }
    ...
    func updateTask(_ newTask: VibeNotchTask) {
        task = newTask
        lastActivity = Date()
    }
