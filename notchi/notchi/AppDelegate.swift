...
private let logger = Logger(subsystem: "com.vibenotch.app", category: "AppDelegate")
...
final class AppDelegate: NSObject, NSApplicationDelegate, SPUUpdaterDelegate, SPUStandardUserDriverDelegate {
    ...
    private lazy var updateUserDriver = VibeNotchUpdateUserDriver(
    ...
    private func startHookServices() {
        ...
        SocketServer.shared.start { event in
            Task { @MainActor in
                VibeNotchStateMachine.shared.handleEvent(event)
            }
        }
    }
